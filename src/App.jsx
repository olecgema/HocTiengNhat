...
function App() {
  ...
  // Function kept for local UI sync, actual DB save happens per answer
    ...
    if (isCorrect) {
      ...
      if (session) {
        ...
        if (!newMap.has(currentItem.char)) {
          ...
          syncLocalMapForUI(newMap);
          // Insert to DB asynchronously (with upsert/unique constraint)
          supabase
            .from('user_mistakes')
            .upsert({
      ...
      {!session ? (
        ...
        <>
          <div className="user-info-banner">
            ...
            <span>Xin chào, <strong>{session.user.email}</strong></span>
          ...
          {/* ===== MENU VIEW ===== */}
          {view === 'MENU' && (
            <main className="view menu-card glass-card">
              <h2>Chọn chế độ học</h2>

              <div className="type-selector">
                <button
                  className={`btn btn-tab ${currentType === 'hiragana' ? 'active' : ''}`}
                  onClick={() => setCurrentType('hiragana')}
                >
                  Hiragana
                </button>
                <button
                  className={`btn btn-tab ${currentType === 'katakana' ? 'active' : ''}`}
              ...
              {isLoading ? (
                <button className="btn btn-secondary pulse-btn" disabled>
          ...
          {view === 'BROWSE' && (
            <main className="view browse-card glass-card">
          ...
          {view === 'QUIZ' && quizPool.length > 0 && (
            <main className="view quiz-card glass-card">
          ...
          {/* ===== RESULTS VIEW ===== */}
          {view === 'RESULTS' && (
            <main className="view results-card glass-card">
              <h2>Kết quả</h2>
              ...
              <div className="results-actions">
                <button onClick={() => setView('MENU')} className="btn btn-secondary">Về Menu</button>
                {sessionMistakes.length > 0 && (
                  <button onClick={() => startQuiz(true)} className="btn btn-primary pulse-btn">Ôn lại ngay</button>
                )}
              </div>
            </main>
          )}
